...
def Squirrel_Search(X_train, y_train, X_test, y_test, n, max_iter, alpha, beta, gamma):
    ...
    for t in range(max_iter):
        ...
        for i in range(n):
            distances[i] = exp(-gamma * np.linalg.norm([positions[i][col] - best_position[col] for col in X_train.columns]) ** 2)
...
# Set the parameters for the Squirrel Search Algorithm
n = 20  # number of squirrels
...
print("Features before Squirrel Search Algorithm: ", len(L))
print(L)
best_position = Squirrel_Search(X_train, y_train, X_test, y_test, n, max_iter, alpha, beta, gamma)
selected_features = [col for col, value in best_position.items() if value == 1]
# selected_cols = [col for col, value in selected_features.items() if value == 1]
print("Features selected by Squirrel Search Algorithm: ", len(selected_features))
print(selected_features)

SSA = selected_features
...
ttsqsa = [X_train, X_test, y_train, y_test]